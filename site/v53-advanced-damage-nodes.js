...
function res(){return {fire:clampRes(num('v53FireRes',30)),cold:clampRes(num('v53ColdRes',30)),lightning:clampRes(num('v53LightRes',30)),chaos:clampRes(num('v53ChaosRes',0))}}
...
function addCritAgainst(c,pct){
  ...
  c.genericCritInc=gi+pct;c.carrierCrit=Math.min(.99,baseCarrier*(1+c.genericCritInc));c.flareCrit=Math.min(.99,baseFlare*(1+c.genericCritInc));
}
function post(parts,rr,pen={},taken=0,fireTaken=0,shock=0){
  const eff={...rr};for(const el of ['fire','cold','lightning'])eff[el]=Math.max(0,eff[el]-(pen[el]||0));
...
function simulate(key){
  ...
  // Pure Power and Lightning Rod are deliberately not forced into a fake exact number.
  if(key==='purePower'||key==='lightningRod')return{n,r,manual:true};
  const totalPost=post(p,res(),pen,taken,fireTaken,shockMag);
  return{n,r,p,totalPost,dps:totalPost*r.real,shockMag};
}
function baseline(){
  const c=baseCfg();if(!c)return null;const r=window.v44Model(c),p=split(c,r),shock=on('v53Shock')?.20:0;
  const d=post(p,res(),{},0,0,shock);return{c,r,p,dps:d*r.real};
}
...
function css(){if($('v53Css'))return;const s=document.createElement('style');s.id='v53Css';s.textContent=`
...
function mount(){
  if($('v53NodeLab'))return;const anchor=$('v52GainLab')||$('v51PackageLab')||$('anoints');if(!anchor)return setTimeout(mount,250);css();const sec=document.createElement('section');sec.id='v53NodeLab';sec.innerHTML=`
  ...
  <div id="v53Top" class="v53Top"></div><div class="tableWrap"><table><thead><tr><th>Node</th><th>Damage mechanism</th><th>Marginal DPS</th><th>Current tree distance</th><th>Liquid recipe</th></tr></thead><tbody id="v53Rows"></tbody></table></div><div id="v53Read" class="v53Read"></div>`;anchor.insertAdjacentElement('afterend',sec);sec.querySelectorAll('input').forEach(e=>{e.addEventListener('input',render);e.addEventListener('change',render)});window.addEventListener('v44calc',render);loadTopo();render();
}
