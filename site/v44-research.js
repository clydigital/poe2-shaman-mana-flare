...
const EXTRA_ITEMS=[
 ['Exceptional 2-socket rare Wand','Wand','Endgame','Preferred non-Runeseeker weapon ceiling. One-handed weapons normally cap at 1 socket; a high-level Exceptional drop can have an additional socket. Buy a strong existing base/item rather than chase-crafting with the last ~15d.','CORE RARE','Verified'],
 ...
 ['Sovereign Alloy wand','Wand','52+','Guaranteed crafted suffix: 20–30% increased effect of Socketed Augment Items. Multiplies both socketed primary augment effects.','ENDGAME','Verified'],
 ['Transcendent Alloy wand','Wand','65+','Current Wand values: 26–31% Cast Speed and Gain 7–11% Elemental Damage as Extra Cold.','CHASE','Verified'],
 ['Perfect Mind Rune','Augment','50','Wand/Staff: +90 maximum Mana; Bonded: 5% increased maximum Mana. Default first rune while maximum Mana is the main bottleneck.','S','Verified'],
 ['Perfect Inspiration Rune','Augment','50','Wand/Staff: 35% increased Mana Regeneration Rate; Bonded: 16% increased Mana Cost Efficiency. Sustain rune — it does NOT grant flat Mana on Wands.','S*','Verified'],
 ['Perfect Storm Rune','Augment','50','Wand/Staff: Gain 12% Damage as Extra Lightning; Bonded: 30% increased Shock magnitude. Payload option after Mana/recovery are healthy.','A*','Verified'],
 ['Kurgal Arcane-Surge rare gloves','Gloves','65+','Desecrated suffix: 10–15% chance to Gain Arcane Surge when you deal a Critical Hit. Preferred budget-endgame Surge source.','CORE RARE','Verified'],
 ['Sacred Flame','Sceptre','55','Shrine Sceptre: 100 Spirit, Gain 40–60% Damage as Extra Fire; enemies in Presence use their lowest elemental resistance for elemental damage. Best weird non-Wand challenger while keeping Rathpith offhand.','TEST+','Verified'],
 ['Guiding Palm of the Mind','Sceptre','65','100 Spirit; Gain 25% Damage as Extra Lightning; +20–30 DEX. Useful Spirit/stat experiment but weaker raw gain than a good Sacred Flame.','TEST','Verified'],
...
function normNode(x){if(x.length>=6)return{name:x[0],kind:x[1],effect:x[2],cat:x[3],grade:x[4],status:x[5]};return{name:x[0],kind:'',effect:x[1],cat:x[2],grade:x[3],status:x[4]}}
function ensureItemFilters(){const sel=$('itemSlot');if(!sel)return;const slots=[...new Set(D.items.concat(EXTRA_ITEMS).map(x=>x[1]).filter(Boolean))].sort();slots.forEach(v=>{if(![...sel.options].some(o=>o.value===v)){const o=document.createElement('option');o.value=v;o.textContent=v;sel.appendChild(o)}})}
function nodeRows(){const h=$('nodeLibrary');if(!h)return;const q=($('nodeSearch')?.value||'').toLowerCase(),cat=$('nodeCat')?.value||'All';const all=D.nodes.map(normNode).concat(EXTRA_NODES.map(normNode)),seen=new Set(),uniq=all.filter(x=>{const k=x.name.toLowerCase();if(seen.has(k))return false;seen.add(k);return true});const a=uniq.filter(x=>(cat==='All'||x.cat===cat)&&(!q||`${x.name} ${x.kind} ${x.effect} ${x.cat} ${x.grade}`.toLowerCase().includes(q)));if($('nodeCount'))$('nodeCount').textContent=`${a.length} / ${uniq.length} nodes`;h.innerHTML=a.map(x=>`<tr><td><b>${x.name}</b>${core.has(x.name)?' <span class="tier test">CURRENT CORE</span>':''}${x.kind?`<small style="display:block;color:#7f7463;margin-top:3px">${x.kind}</small>`:''}</td><td>${tag(x.cat)}</td><td>${x.effect}</td><td><b>${x.grade}</b></td><td class="${x.status==='Verified'?'statusVerified':'statusResearch'}">${x.status}</td></tr>`).join('')}
function itemRows(){const h=$('itemLibrary');if(!h)return;ensureItemFilters();const q=($('itemSearch')?.value||'').toLowerCase(),slot=$('itemSlot')?.value||'All';const all=D.items.concat(EXTRA_ITEMS),seen=new Set(),uniq=all.filter(x=>{const k=x[0].toLowerCase();if(seen.has(k))return false;seen.add(k);return true});const a=uniq.map(x=>({name:x[0],slot:x[1],level:x[2],role:x[3],grade:x[4],status:x[5]})).filter(x=>(slot==='All'||x.slot===slot)&&(!q||`${x.name} ${x.slot} ${x.role} ${x.grade}`.toLowerCase().includes(q)));if($('itemCount'))$('itemCount').textContent=`${a.length} / ${uniq.length} items`;h.innerHTML=a.map(x=>`<tr><td><b>${x.name}</b></td><td>${x.slot}</td><td>${x.level}</td><td>${x.role}</td><td><b>${x.grade}</b></td><td class="${x.status==='Verified'?'statusVerified':'statusResearch'}">${x.status}</td></tr>`).join('')}
...
ensureItemFilters();['nodeSearch','nodeCat'].forEach(id=>$(id)?.addEventListener(id==='nodeCat'?'change':'input',nodeRows));['itemSearch','itemSlot'].forEach(id=>$(id)?.addEventListener(id==='itemSlot'?'change':'input',itemRows));window.v44ResearchRenderItems=itemRows;window.v44ResearchRenderNodes=nodeRows;nodeRows();itemRows();flaskRows();
})();
